...
export default function DataTableCrudDemo({ facturas, facturaId, formDataValues, facturaEnviada, dataValuesProducts }) {
    ...
    useEffect(() => {
        setProducts(facturas);
         ...
         <DetallePaciente updateComponent={facturas}/>
    }, [products, product, facturas, submitted]);
    useEffect(() => {
    ...
    const saveProduct = () => {
        setSubmitted(true);
        let id = product.id
        let pacienteId = facturaId.id

        let _products = [...products];
        let _product = { ...product, id, pacienteId };
 ...
 */}            </React.Fragment>
    ...
    console.log(dataValuesProducts)
    ...
    return (
        <div className="datatable-crud-demo">
            ...
            {dataValuesProducts !== null ? <Dialog visible={uploadFacturaDialog} style={{ width: '450px' }} header={` Nueva factura paciente:  ${dataValuesProducts.nombre} ${dataValuesProducts.apellido}`} modal className="p-fluid" footer={subirFacturaDialog} onHide={hideDialog} >
